...
mod tests_int {
    ...
    fn test_execute_expand_modifiers_absolute() {
        ...
        assert_eq!(Err("inventory_index 0 and index_specifier Absolute(0) cannot be the same".to_string()), execute_craft_expand_modifiers(&mut game, 0, vec![IndexSpecifier::Absolute(0), IndexSpecifier::Absolute(1)]));
        assert_eq!(1, game.inventory[0].as_ref().unwrap().modifiers.len());
        ...
        assert_eq!(Err("inventory_index 99 is not within the range of the inventory 9".to_string()), execute_craft_expand_modifiers(&mut game, 99, vec![IndexSpecifier::Absolute(1), IndexSpecifier::Absolute(2)]));
        assert_eq!(Err("index_specifier Absolute(99) is not within the range of the inventory 9".to_string()), execute_craft_expand_modifiers(&mut game, 0, vec![IndexSpecifier::Absolute(99), IndexSpecifier::Absolute(1), IndexSpecifier::Absolute(2), IndexSpecifier::Absolute(3)]));
        assert_eq!(Err("inventory_index 1 is empty.".to_string()), execute_craft_expand_modifiers(&mut game, 1, vec![IndexSpecifier::Absolute(1), IndexSpecifier::Absolute(2), IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(4)]));
        assert_eq!(Err("index_specifier Absolute(1) is pointing at empty inventory slot.".to_string()), execute_craft_expand_modifiers(&mut game, 0, vec![IndexSpecifier::Absolute(1), IndexSpecifier::Absolute(2), IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(4)]));
        assert_eq!(Err("index_specifier Absolute(3) is already present in calculated sacrifice indexes [3]".to_string()), execute_craft_expand_modifiers(&mut game, 0, vec![IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(4)]));
        assert_eq!(Err("sacrifice_item_index 3 need to have at least 2 modifiers but it only had 1".to_string()), execute_craft_expand_modifiers(&mut game, 0, vec![IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(4), IndexSpecifier::Absolute(5), IndexSpecifier::Absolute(6)]));
        ...
        assert_eq!(Err("inventory_index.possible_rolls.min_simultaneous_resistances 0 need to be bigger than inventory_index current number of modifiers 2 for it to be expanded.".to_string()), execute_craft_expand_modifiers(&mut game, 0, vec![IndexSpecifier::Absolute(1), IndexSpecifier::Absolute(2)]));
    }
    ...
    fn test_execute_expand_modifiers_relative_positive() {
        ...
        assert_eq!(Err("index_specifier: RelativePositive(1) did not find any items in inventory from relative point 7 until end of inventory.".to_string()), execute_craft_expand_modifiers(&mut game, 6, vec![IndexSpecifier::RelativePositive(1), IndexSpecifier::RelativePositive(1)]));
    }
    ...
    fn test_execute_expand_modifiers_relative_negative() {
        ...
        assert_eq!(Err("index_specifier: RelativeNegative(1) did not find any items in inventory from relative point 3 until start of inventory.".to_string()), execute_craft_expand_modifiers(&mut game, 2, vec![IndexSpecifier::RelativeNegative(1), IndexSpecifier::RelativeNegative(1)]));
    }
    ...
    fn test_execute_expand_modifiers_relative_mix() {
        ...
        game.inventory[1] = None;
        game.inventory[0] = None;
        assert_eq!(Err("index_specifier: RelativeNegative(1) did not find any items in inventory from relative point 3 until start of inventory.".to_string()), execute_craft_expand_modifiers(&mut game, 2, vec![IndexSpecifier::RelativeNegative(1), IndexSpecifier::RelativePositive(1)]));
        assert_eq!(Err("index_specifier: RelativeNegative(1) did not find any items in inventory from relative point 3 until start of inventory.".to_string()), execute_craft_expand_modifiers(&mut game, 2, vec![IndexSpecifier::RelativePositive(1), IndexSpecifier::RelativeNegative(1)]));

        game.inventory[8] = None;
        assert_eq!(Err("index_specifier: RelativePositive(1) did not find any items in inventory from relative point 3 until end of inventory.".to_string()), execute_craft_expand_modifiers(&mut game, 2, vec![IndexSpecifier::RelativePositive(1), IndexSpecifier::RelativeNegative(1)]));
    }
