...
pub fn execute(game: &mut Game) -> Result<ExecuteExpandMaxSimultaneousElementReport, MyError> {
    if (game.difficulty.max_simultaneous_resistances as usize) >= game.difficulty.max_resistance.len() {
        return Err(MyError::create_execute_command_error(format!("max_simultaneous_resistances {} is already equal to number of active resistances {}. Consider calling ExpandElements.", game.difficulty.max_simultaneous_resistances, game.difficulty.max_resistance.len())));
    }
...
mod tests_int {
    ...
    fn test_execute_expand_max_simultaneous_element() {
        ...
        assert_eq!(Err(MyError::create_execute_command_error("max_simultaneous_resistances 1 is already equal to number of active resistances 1. Consider calling ExpandElements.".to_string())), execute_expand_max_simultaneous_element(&mut game));
        ...
        *game.treasure.get_mut(&Gold).unwrap() += 1000;
        ...
        assert_eq!(Err(MyError::create_execute_command_error("max_simultaneous_resistances 2 is already equal to number of active resistances 2. Consider calling ExpandElements.".to_string())), execute_expand_max_simultaneous_element(&mut game));
        assert_eq!(2, game.difficulty.max_resistance.len());
        assert_eq!(2, game.difficulty.min_resistance.len());
    }
}