...
mod tests_int {
    ...
    fn test_execute_expand_modifiers_absolute() {
        ...
        assert_eq!(Err(MyError::create_execute_command_error("sacrifice_item_index 3 need to have at least 2 modifiers but it only had 1".to_string())), execute_craft_expand_modifiers(&mut game, 0, vec![IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(3), IndexSpecifier::Absolute(4)]));
