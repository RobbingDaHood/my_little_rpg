...
mod tests_int {
    ...
    fn test_execute_craft_item() {
        ...
        assert_eq!(Err("inventory_index 0 and index_specifier Absolute(0) cannot be the same".to_string()), execute(&mut game, 0, 0, vec![index_specifier::IndexSpecifier::Absolute(0), index_specifier::IndexSpecifier::Absolute(1)]));
        assert_eq!(old_item, game.inventory[0]);
        assert_eq!(10, game.inventory.len());

        assert_eq!(Err("sacrifice_item_index 1 need to have at least 2 modifiers but it only had 1".to_string()), execute(&mut game, 0, 1, vec![index_specifier::IndexSpecifier::Absolute(1), index_specifier::IndexSpecifier::Absolute(2)]));
        assert_eq!(old_item, game.inventory[0]);
        assert_eq!(10, game.inventory.len());

        assert_eq!(2, execute_craft_reroll_modifier_calculate_cost(&game, 0));
        assert_eq!(Err("craft_reroll_modifier needs 2 items to be sacrificed but you only provided 1".to_string()), execute(&mut game, 0, 0, vec![index_specifier::IndexSpecifier::Absolute(1)]));
        assert_eq!(old_item, game.inventory[0]);
        assert_eq!(10, game.inventory.len());
        ...
        assert_eq!(Err("inventory_index 99 is not within the range of the inventory 10".to_string()), execute(&mut game, 99, 0, vec![index_specifier::IndexSpecifier::Absolute(0)]));
        assert_eq!(Err("modifier_index 99 is not within the range of the item modifiers 2".to_string()), execute(&mut game, 0, 99, vec![index_specifier::IndexSpecifier::Absolute(0)]));
        assert_eq!(Err("index_specifier Absolute(99) is not within the range of the inventory 10".to_string()), execute(&mut game, 0, 0, vec![index_specifier::IndexSpecifier::Absolute(99), index_specifier::IndexSpecifier::Absolute(1)]));
        assert_eq!(Err("inventory_index 1 is empty.".to_string()), execute(&mut game, 1, 0, vec![index_specifier::IndexSpecifier::Absolute(4), index_specifier::IndexSpecifier::Absolute(5)]));
        assert_eq!(Err("index_specifier Absolute(1) is pointing at empty inventory slot.".to_string()), execute(&mut game, 0, 0, vec![index_specifier::IndexSpecifier::Absolute(1), index_specifier::IndexSpecifier::Absolute(2)]));
        assert_eq!(Err("index_specifier Absolute(8) is already present in calculated sacrifice indexes [8]".to_string()), execute(&mut game, 0, 0, vec![index_specifier::IndexSpecifier::Absolute(8), index_specifier::IndexSpecifier::Absolute(8)]));
        assert_eq!(old_item, game.inventory[0]);
        assert_eq!(8, game.inventory.iter().filter(|i| i.is_some()).count());
    }
    ...
    fn test_execute_craft_item_positive() {
        ...
        assert_eq!(Err("index_specifier: RelativePositive(1) did not find any items in inventory from relative point 1 until end of inventory.".to_string()), execute(&mut game, 0, 0, vec![index_specifier::IndexSpecifier::RelativePositive(1), index_specifier::IndexSpecifier::RelativePositive(1)]));
        assert_eq!(2, game.inventory.iter().filter(|i| i.is_some()).count());
    }
    ...
    fn test_execute_craft_item_negative() {
        ...
        assert_eq!(Err("index_specifier: RelativeNegative(1) did not find any items in inventory from relative point 10 until start of inventory.".to_string()), execute(&mut game, 9, 0, vec![index_specifier::IndexSpecifier::RelativeNegative(1), index_specifier::IndexSpecifier::RelativeNegative(1)]));
        assert_eq!(2, game.inventory.iter().filter(|i| i.is_some()).count());
    }
    ...
    fn test_execute_craft_item_mixed() {
        ...
        assert_eq!(Err("index_specifier: RelativePositive(1) did not find any items in inventory from relative point 6 until end of inventory.".to_string()), execute(&mut game, 5, 0, vec![index_specifier::IndexSpecifier::RelativePositive(1), index_specifier::IndexSpecifier::RelativeNegative(1)]));
        assert_eq!(Err("index_specifier: RelativePositive(1) did not find any items in inventory from relative point 6 until end of inventory.".to_string()), execute(&mut game, 5, 0, vec![index_specifier::IndexSpecifier::RelativeNegative(1), index_specifier::IndexSpecifier::RelativePositive(1)]));
        assert_eq!(2, game.inventory.iter().filter(|i| i.is_some()).count());

        game.inventory[2] = None;
        assert_eq!(Err("index_specifier: RelativeNegative(1) did not find any items in inventory from relative point 6 until start of inventory.".to_string()), execute(&mut game, 5, 0, vec![index_specifier::IndexSpecifier::RelativeNegative(1), index_specifier::IndexSpecifier::RelativeNegative(1)]));
    }
