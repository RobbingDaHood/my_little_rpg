...
pub fn execute(game: &mut Game, inventory_index: usize, modifier_index: usize, mut sacrifice_item_indexes: Vec<IndexSpecifier>) -> Result<ExecuteCraftRerollModifierReport, MyError> {
    ...
    if inventory_item.modifiers.len() <= modifier_index {
        return Err(MyError::create_execute_command_error(format!("modifier_index {} is not within the range of the item modifiers {}", modifier_index.clone(), inventory_item.modifiers.len())));
    }
...
fn get_index_specifier_error_conditions(modifier_index: usize) -> ErrorConditions {
    let enough_modifiers_condition = move |sacrifice_item_index: usize, sacrificed_item: &Item| {
        if sacrificed_item.modifiers.len() <= modifier_index {
            Some(MyError::create_execute_command_error(format!("sacrifice_item_index {} need to have at least {} modifiers but it only had {}", sacrifice_item_index, modifier_index + 1, sacrificed_item.modifiers.len())))
        } else {
            None
        }
    };
