...
fn execute_help_for_command(command: Command) -> &'static str {
    match command {
        ...
        Command::Move(_) => "Move X: Move to place with index X. The game will tell you if you win the place and get the reward, with a lot more info too. After the move the place would be reroled. You can move to the same place as many times in a row you want.",
        ...
        Command::RerolModifier(_, _, _) => "RerolModifier X Y Z: Rerol the modifier at index Y oo item in inventory at index X. Z is a comma seperated list of indexes for items in the inventory to sacrifice to pay the cost of the command, each item need to have at least the same amount of modifiers as the index of the modifier being reroled. Z can contain relative indexes prefixed with + or -, they are relative to X.",
